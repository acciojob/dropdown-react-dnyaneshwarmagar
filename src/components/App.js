    ...
    city: [
      {
        ...
        landmarks: [
          ...
          },
          {
            name: "Maner",
            description:
              "Maner also spelled Maner or Maner Sharif, is a satellite landmark in Patna Metropolitan Region, in the Patna district in the Indian state of Bihar.",
          },
        ],
      },
      {
...
function App() {
  ...
  const handleCityChange = (event) => {
    const newCity = parseInt(event.target.value);
    setSelectedCity(newCity);
    setSelectedLandmark(0);
  };

  const handleLandmarkChange = (event) => {
    const newLandmark = parseInt(event.target.value);
